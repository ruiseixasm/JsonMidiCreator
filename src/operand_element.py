...
class ControlChangeLSB(ControlChange):
    def __init__(self, *parameters):
        self._lsb: int = 0
        super().__init__(*parameters)

    def lsb(self, number: Optional[int] = None, value: Optional[int] = None) -> Self:
        self._lsb = og.Controller(
                ou.Number(number), ou.Value(value)
            )
        return self
    ...
    def __mod__(self, operand: o.T) -> o.T:
        ...
        Examples
        --------
        >>> lsb = Controller("Modulation")
        >>> lsb % Number() % int() >> Print()
        1
        """
        match operand:
            case od.DataSource():
                match operand._data:
                    case og.Controller():       return self._lsb
                    case _:                     return super().__mod__(operand)
            case og.Controller():       return self._lsb.copy()
            case ou.Number():           return self._lsb % operand
            case ou.Value():            return self._lsb % operand
            case int() | float():       return self._lsb % operand
            case _:                     return super().__mod__(operand)
    ...
    def __eq__(self, other: o.Operand) -> bool:
        ...
        match other:
            case self.__class__():
                return super().__eq__(other) \
                    and self._lsb == other % od.DataSource( og.Controller() )
            case _:
                return super().__eq__(other)
        ...
        return [
                {
                    ...
                    "midi_message": {
                        "status_byte": 0xB0 | 0x0F & self._channel - 1,
                        "data_byte_1": self._lsb._number,
                        "data_byte_2": self._lsb._value,
                        "device": self._device
                    }
                }
            ]
    ...
    def getMidilist(self, midi_track: ou.MidiTrack = None, position_beats: Fraction = None) -> list:
        ...
        self_midilist[0]["event"]       = "ControllerEvent"
        # Validation is done by midiutil Midi Range Validation
        self_midilist[0]["number"]      = self._lsb._number
        self_midilist[0]["value"]       = self._lsb._value
        return self_midilist
    ...
    def __lshift__(self, operand: any) -> Self:
        ...
        match operand:
            ...
            case od.DataSource():
                match operand._data:
                    case og.Controller():       self._lsb = operand._data
                    case _:                     super().__lshift__(operand)
            case og.Controller() | ou.Number() | ou.Value() | int() | float() | str():
                self._lsb << operand
            case _: super().__lshift__(operand)
    ...
    def __iadd__(self, operand: any) -> 'ControlChange':
        ...
        match operand:
            case int() | float() | ou.Value():
                self._lsb += operand  # Specific and compounded parameter
                return self
            case _:
                return super().__iadd__(operand)
