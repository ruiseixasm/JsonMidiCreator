                ...
                if self._owner_clip is not None:
                    if not self._owner_clip._set:
                        if operand.len() > 0:
                            ...
                            elements_list: list[Element] = [
                                (element + position_offset)._set_owner_clip(self._owner_clip) for element in operand._foreground_items()
                            ]
                            self._owner_clip._extend(elements_list)
                        self._owner_clip._set = True
                    self._owner_clip._remove(self, True)
...
class C2(Chord):
    """`Element -> DeviceElement -> ChannelElement -> Note -> KeyScale -> Chord -> C2`

    A `C2` is a Chord with a default `NoteValue` of `1/2`, also known as half-note chord.
...
class C4(Chord):
    """`Element -> DeviceElement -> ChannelElement -> Note -> KeyScale -> Chord -> C4`

    A `C4` is a Chord with a default `NoteValue` of `1/4`, also known as quarter-note chord.
...
class C8(Chord):
    """`Element -> DeviceElement -> ChannelElement -> Note -> KeyScale -> Chord -> C8`

    A `C8` is a Chord with a default `NoteValue` of `1/8`, also known as eight-note chord.
...
class C16(Chord):
    """`Element -> DeviceElement -> ChannelElement -> Note -> KeyScale -> Chord -> C16`

    A `C16` is a Chord with a default `NoteValue` of `1/16`, also known as sixteenth-note chord.
