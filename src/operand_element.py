...
class Note(ChannelElement):
    ...
    def checksum(self) -> int:
        ...
        master: int = self._velocity << 7 + 4 | self._pitch._chromatic_pitch_int() << 4 | self._channel_0
        master ^= self._position_beats.numerator << 8 | self._position_beats.denominator
        master ^= self._duration_beats.numerator << 8 | self._duration_beats.denominator
        return master & 0xFFFF  # 16-bit

    def center_pitch(self) -> int:
        return self._pitch._chromatic_pitch_int()
    ...
    def __lt__(self, other: 'o.Operand') -> bool:
        match other:
            case Note():
                ...
                if self._position_beats == other._position_beats:
                    self_pitch: int = self._pitch._chromatic_pitch_int()
                    other_pitch: int = other._pitch._chromatic_pitch_int()
    ...
    def __mod__(self, operand: o.T) -> o.T:
        ...
        match operand:
            ...
            case ou.PitchParameter() | ou.Natural() | ou.Quality() | str() | og.Scale():
                                    return self._pitch % operand
            case ou.DrumKit():
                return ou.DrumKit(self._pitch._chromatic_pitch_int(), ou.Channel(self._channel_0 + 1))
            case _:                 return super().__mod__(operand)
    ...
    def getPlotlist(self,
        ...
        pitch_int: int = self._pitch._chromatic_pitch_int()
...
class KeyScale(Note):
    ...
    def center_pitch(self) -> int:
        pitches: list[int] = [
            single_note._pitch._chromatic_pitch_int() for single_note in self.get_component_elements()
        ]
        if pitches:
            total = sum(pitches)
            count = len(pitches)
    ...
    def getPlotlist(self,
        ...
        if self_plotlist:
            self_plotlist[-1]["note"]["middle_pitch"] = self._pitch._chromatic_pitch_int()
        return self_plotlist
