...
class Composition(Container):
    ...
    def _plot_elements(self):
        ...
        if isinstance(self, Block):
            ...
            chart_title += " - "
        chart_title += f"{"Mask - " if self._compositions[self._iteration].is_masked() else ""}"
        ...
        if event.button == 3 and event.xdata is not None and event.ydata is not None:   # 1=left, 2=middle, 3=right
            ...
            if at_position_notes:
                if self._by_channel:
                    ...
                    at_position_notes.sort(key=lambda note:note._pitch._chromatic_pitch())
                    ...
                    for single_note in at_position_notes:
                        if minimum_position is None:
                            ...
                            root_pitch: int = single_note._pitch._chromatic_pitch()
    ...
    def plot(self, by_channel: bool = False, block: bool = True, pause: float = 0, iterations: int = 0,
        ...
        self._compositions: list[Composition] = [ self.copy() ]   # Works with a forced copy (Read Only)
...
class Clip(Composition):  # Just a container of Elements
    ...
    def mirror(self, by_degree: bool = True) -> Self:
        ...
        if by_degree:
            ...
            if higher_pitch is not None:
                ...
                top_pitch_int: int = higher_pitch._chromatic_pitch()
                bottom_pitch_int: int = lower_pitch._chromatic_pitch()

                for element in self._foreground_items():
                    if isinstance(element, oe.Note):
                        note_pitch: og.Pitch = element._pitch
                        note_pitch_int: int = note_pitch._chromatic_pitch()
    ...
    def invert(self, by_degree: bool = True) -> Self:
        ...
        if by_degree:
            ...
            for note in self._foreground_items():
                if isinstance(note, oe.Note):
                    center_pitch = note._pitch._chromatic_pitch()
                    break

            for note in self._foreground_items():
                if isinstance(note, oe.Note):
                    note_pitch: int = note._pitch._chromatic_pitch()
    ...
    def join(self, decompose: bool = True) -> Self:
        ...
        for note in all_notes:
            channel_pitch: int = note._channel_0 << 8 | note._pitch._chromatic_pitch()
