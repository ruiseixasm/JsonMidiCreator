...
class Single(Generic):
    ...
    def __mod__(self, operand: any) -> any:
        match operand:
            ...
            case _:
                if isinstance(self._parameter, (o.Operand, list)):
    ...
    def __gt__(self, other: 'Single') -> bool:
        other = self & other    # Processes the tailed self operands or the Frame operand if any exists
        if type(self) != type(other):
            return False
        return  self._parameter > other._parameter
    ...
    def getSerialization(self) -> dict:
        device_serialization = super().getSerialization()
        device_serialization["parameters"]["parameter"] = self._parameter
        if isinstance(self._parameter, o.Operand):
            device_serialization["parameters"]["parameter"] = self._parameter.getSerialization()
        return device_serialization

    # CHAINABLE OPERATIONS

    def loadSerialization(self, serialization: dict) -> 'Single':
        if isinstance(serialization, dict) and ("class" in serialization and serialization["class"] == self.__class__.__name__ and "parameters" in serialization and
            "parameter" in serialization["parameters"]):
            
            super().loadSerialization(serialization)
            self._parameter = serialization["parameters"]["parameter"]
            if isinstance(serialization["parameters"]["parameter"], dict) and "class" in serialization["parameters"]["parameter"]:
                self._parameter = o.Operand.loadSerialization(serialization["parameters"]["parameter"])
        return self
        
    def __lshift__(self, operand: any) -> 'Single':
        operand = self & operand    # Processes the tailed self operands or the Frame operand if any exists
        match operand:
            case od.DataSource():
                self._parameter = operand % o.Operand()
            case Single():
                super().__lshift__(operand)
                if isinstance(operand._parameter, (o.Operand, list)):
