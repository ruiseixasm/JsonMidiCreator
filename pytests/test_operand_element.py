...
def test_chord_mod():
    ...
    E_minor_triad: Chord = Chord()  # As is it has no Scale, so, it uses the KeySignature
    settings << KeySignature() # Resets the default Key Scale to C Major being used by the triad (unowned by any clip)
    # Now we are using the E Major given the C Major Key Signature by changing the triad KeyScale itself
    E_minor_triad <<= Major()   # This change the scale to Major!
    print("------")
    three_notes = E_minor_triad.get_component_elements()
    print(f"Key: {three_notes[0] % TargetKey() % str()}")
    assert three_notes[0] % TargetKey() == "E"
    print(f"Key: {three_notes[1] % TargetKey() % str()}")
...
def test_note3_element():
    ...
    assert triplet_note % od.Pipe( Duration() ) == NoteValue(1/16)
    assert triplet_note % Position() == 0.0

    assert (Triplet() << (NoteValue() << Duration(1/16))) % NoteValue() == Duration(1/16)
    assert (Triplet() << (NoteValue() << Duration(1/16))) % od.Pipe( Duration() ) == NoteValue(1/16)
    assert (Triplet() << (NoteValue() << Duration(1/16))) % Position() == 0.0
...
def test_note_pitch():
    ...
    print(f"Key: {(note_copy_2) % str()}, \
            tone: {note_copy_2 % Pitch() % Pipe(Key()) % int()}, degree: {(note_copy_2) % Degree() % int()}")
...
def test_chord_element():
    ...
    for key in range(3):
        triad_notes[key] % Key() % str() >> Print()
        assert triad_notes[key] % Key() % str() == expected_keys[key]

    # WITH DEGREE
    print("------")
    triad << Degree("ii") << Scale([])  # Uses the Staff signature (Major Scale)
    assert Note(triad) % Key() % str() == "D"
