...
def test_rshift_container():
    ...
    assert all_chords[0].access(Pitch())._chromatic_pitch_int() == 60
    ...
    assert first_chords[0].access(Pitch())._chromatic_pitch_int() == 60
    first_chords << Degree(5) << Mode(5)
    assert first_chords is not all_chords
    assert all_chords._test_owner_clip()
    assert first_chords._test_owner_clip()
    # assert first_chords[0] is not all_chords[0]
    assert first_chords[0].access(Pitch())._chromatic_pitch_int() == 67
...
def test_checksum():
    ...
    assert checksum_to_string(song_notes.checksum()) == "8900"
...
def test_floordiv_clip():
    ...
    just_rests_mask = Rest(1/2) / 2 >> Beat(2)  # 1st beat masked, meaning, 2nd Rest
