...
multi_notes = Rest(Duration(Steps(3*4 + 2))) >> (first_note + Rest()) * 3 << MidiTrack(1, "Piano") >> Save("json/testing/_Save_Play_p.1_first_note.json") >> Export("json/testing/_Export_Play_p.1_sequence.json") \
...
single_clock << Duration(Measures(2)) >> triplets_one >> triplets_two >> Save("json/testing/_Save_Play_p.10_first_note.json") >> Export("json/testing/_Export_Play_p.10_sequence.json")
