...
class Element(o.Operand):
    ...
    def getSerialization(self) -> dict:
        ...
        if self._masked:    # Cleans up the saved json from non musical tags
            serialization["parameters"]["masked"]           = o.serialize(self._masked)
                    ...
                    if not self._owner_clip._set:
                        if operand.len() > 0:
                            ...
                            elements_list: list[Element] = [
                                ...
                                for element in operand.elements_unmasked()
                            ]
                            self._owner_clip._extend(elements_list)
                        self._owner_clip._set = True
                    self._owner_clip._remove(self, True)
...
class Subclip(Element):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["subclip"] = o.serialize( self._subclip )
        return serialization
    ...
    def __lshift__(self, operand: any) -> Self:
        ...
        match operand:
            ...
            case oc.Clip():
                ...
                self << ra.Duration(self._subclip.net_finish()) # Updates the Element Duration
...
class Unison(Element):
    ...
    def getPlaylist(self, position_beats: Fraction | None = None, devices_header = True) -> list[dict]:
        ...
        if not isinstance(position_beats, Fraction):
            ...
            return []
        for single_element in self.get_component_elements():
            self_playlist.extend(single_element.getPlaylist(position_beats, devices_header))
        return self_playlist
    ...
    def getMidilist(self, position_beats: Fraction | None = None) -> list[dict]:
        ...
        if not isinstance(position_beats, Fraction):
            ...
            return []
        for single_element in self.get_component_elements():
            self_midilist.extend(single_element.getMidilist(position_beats))    # extends the list with other list
        return self_midilist
    
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["elements"] = o.serialize( self._elements )
        return serialization
    ...
    def __lshift__(self, operand: any) -> Self:
        ...
        match operand:
            case Unison():
                super().__lshift__(operand)
                self._elements = o.deep_copy( operand._elements )
            case od.Pipe():
                match operand._data:
                    case od.Elements():
            ...
            case od.Elements():
                if all(isinstance(single_element, Element) for single_element in operand._data):
                    self._elements = o.deep_copy(list(operand._data))
            case dict():
...
class Talkie(Element):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["port"]         = o.serialize(self._port)
        serialization["parameters"]["to"]           = o.serialize(self._to)
        serialization["parameters"]["channel"]      = o.serialize(self._channel_0)
        return serialization
...
class TalkieRun(Talkie):
    ...
    def getPlaylist(self, position_beats: Fraction | None = None, devices_header = True) -> list[dict]:
        ...
        if not isinstance(position_beats, Fraction):
            ...
            return []
        self_playlist: list[dict] = super().getPlaylist(position_beats, devices_header)
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["name"]           = o.serialize(self._name)
        return serialization
...
class DeviceElement(Element):
    ...
    def getMidilist(self, position_beats: Fraction | None = None, derived_element: 'Element' = None) -> list:
        ...
        if self._owner_clip is not None:
            track_number = self._owner_clip._track_number
            track_name = self._owner_clip._name

        # Validation is done by midiutil Midi Range Validation
        return [
                {
                    "event":        "Element",
                    "track":        track_number - 1,   # out of range shouldn't be exported as a midi track
            ...
            self_plotlist.append(
                {
                    "note": {
                        ...
                        "sharps": single_note._pitch.get_sharps(),
                        "mode": single_note._pitch._diatonic_mode_0,
                        "key_tonic": single_note._pitch._tonic_key,
...
class Rhythm(Note):
    ...
    def getPlaylist(self, position_beats: Fraction | None = None, devices_header = True) -> list[dict]:
        ...
        if not isinstance(position_beats, Fraction):
            ...
            return []
        for single_note in self.get_component_elements():
            self_playlist.extend(single_note.getPlaylist(position_beats, devices_header))
        return self_playlist
    ...
    def getMidilist(self, position_beats: Fraction | None = None) -> list[dict]:
        ...
        if not isinstance(position_beats, Fraction):
            ...
            return []
        for single_note in self.get_component_elements():
            self_midilist.extend(single_note.getMidilist(position_beats))
        return self_midilist
...
class Quadruplet(Rhythm):
    """`Element -> DeviceElement -> ChannelElement -> Note -> Rhythm -> Quadruplet`

    A `Quadruplet` is the given Note split four times resulting in each single resulting note with a 1/4 of the original duration.
    ...
    def get_component_elements(self) -> list[Note]:
        ...
        third_note = Note(second_note)
        third_note._position_beats += second_note._duration_beats
        fourth_note = Note(third_note)
        fourth_note._position_beats += third_note._duration_beats
        return [first_note, second_note, third_note, fourth_note]
...
class Galop(Rhythm):
    ...
    def get_component_elements(self) -> list[Note]:
        """Returns the elements directly, NO decoupling guaranteed (no copy)"""
        first_note = Note(self)
        first_note._duration_beats /= 2 # Converts to 1/8 alike
        second_note = Note(first_note)
        second_note._position_beats += first_note._duration_beats
...
class KeyScale(Note):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["inversion"]    = o.serialize( self._inversion )
        return serialization
...
class Cluster(KeyScale):
        ...
        match operand:
            case od.Pipe():
                match operand._data:
                    ...
                    case _:                 return super().__mod__(operand)
            case list():            return o.deep_copy(self._pitches)
            case _:                 return super().__mod__(operand)
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["pitches"] = o.serialize( self._pitches )
        return serialization
    ...
    def __lshift__(self, operand: any) -> Self:
        ...
        match operand:
            case Cluster():
                super().__lshift__(operand)
                self._pitches = o.deep_copy( operand._pitches )
            case od.Pipe():
                match operand._data:
                    case list():
            ...
            case list():
                if all(not isinstance(item, Element) for item in operand):
                    self._pitches = o.deep_copy( operand )
                else:
                    super().__lshift__(operand)
            case dict():
                if all(isinstance(key, int) for key in operand.keys()):
                    for index, offset in operand.items():
                        if index >= 0 and index < len(self._pitches):
                            self._pitches[index] = o.deep_copy(offset)
                else:   # Not for me
                    self._pitch << operand
            case _:
                super().__lshift__(operand)
...
class Chord(KeyScale):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["size"]         = o.serialize( self._size )
        serialization["parameters"]["dominant"]     = o.serialize( self._dominant )
        serialization["parameters"]["diminished"]   = o.serialize( self._diminished )
        serialization["parameters"]["augmented"]    = o.serialize( self._augmented )
        serialization["parameters"]["sus2"]         = o.serialize( self._sus2 )
        serialization["parameters"]["sus4"]         = o.serialize( self._sus4 )
        return serialization
...
class Tuplet(Note):
    ...
    def getMidilist(self, position_beats: Fraction | None = None) -> list[dict]:
        ...
        if not isinstance(position_beats, Fraction):
            ...
            return []
        for single_note in self.get_component_elements():
            self_midilist.extend(single_note.getMidilist(position_beats))    # extends the list with other list
        return self_midilist
    
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["count"]    = o.serialize( self._count )
...
class ControlChange(Automatable):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["number"]   = o.serialize( self._number )
        serialization["parameters"]["value"]    = o.serialize( self._value )
        return serialization
...
class ControlChangePair(ControlChange):
    ...
    def getPlaylist(self, position_beats: Fraction | None = None, devices_header = True) -> list[dict]:
        ...
        if self_playlist:   # The second list to set it as LSB
            self_playlist_lsb: list[dict] = o.deep_copy(self_playlist)
            self_playlist_lsb[0]["midi_message"]["data_byte_1"] = self._number_lsb
            self_playlist_lsb[0]["midi_message"]["data_byte_2"] = clamp_value_128(self._value_lsb)
    ...
    def getMidilist(self, position_beats: Fraction | None = None) -> list[dict]:
        ...
        if self_midilist:   # The second list to set it as LSB
            self_midilist_lsb: list[dict] = o.deep_copy(self_midilist)
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["number_lsb"]   = o.serialize( self._number_lsb )
        serialization["parameters"]["value_lsb"]    = o.serialize( self._value_lsb )
        return serialization
...
class Aftertouch(Automatable):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["pressure"] = o.serialize( self._pressure )
        return serialization
...
class PitchBend(Automatable):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["msb"] = o.serialize(self._msb)
...
class Automation(Element):
    ...
    def smoothstep_fraction(t: Fraction) -> Fraction:
        """
        t: Fraction in [0, 1]
        Returns smoothstep value as Fraction
        """
        # t²(3-2t) as fractions
        t2: Fraction = t * t
    ...
    def __mod__(self, operand: o.T) -> o.T:
        match operand:
            ...
            case list():            return o.deep_copy(self._list)
    ...
    def getSerialization(self) -> dict:
        ...
        serialization["parameters"]["linear"]       = o.serialize( self._linear )
        return serialization
    ...
    def __lshift__(self, operand: any) -> Self:
        ...
        match operand:
            case Automation():
                ...
                self._dots = o.deep_copy(operand._dots)
...
class ProgramChange(ChannelElement):
    ...
    def getSerialization(self) -> dict:
        serialization = super().getSerialization()
        serialization["parameters"]["program_0"]    = o.serialize( self._program_0 )
        return serialization
    ...
    def getPlaylist(self, position_beats: Fraction | None = None, devices_header = True) -> list[dict]:
        ...
        for channel in range(1, 17):
            ...
            self_playlist.extend(AllNotesOff(self, ou.Channel(channel))
                .getPlaylist(position_beats, False))
