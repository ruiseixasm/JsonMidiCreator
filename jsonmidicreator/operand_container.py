...
class Container(o.Operand):
    ...
    def __getitem__(self, index: Any) -> any:
        items_unmasked = self.items_unmasked()
        match index:
            case of.Frame():
    ...
    def __iadd__(self, operand: any) -> Self:
        match operand:
            case Container():
                return self._extend( o.deep_copy(operand._items) )
            case list():
                return self._extend( o.deep_copy(operand) )
...
class Composition(Container):
    ...
    def start(self) -> 'ra.Position':
        net_start = self.net_start()
        if net_start is not None:
            return net_start.roundMeasures()
        return None
...
class Clip(Composition):  # Just a container of Elements
    ...
    def __setitem__(self, index: Any, value: 'oe.Element') -> Self:
        """
    ...
    def get_component_elements(self) -> list['oe.Element']:
        ...
        note_extended_durations: list[Fraction] = [Fraction(0)] * len(component_notes)
        for index, clip_note in enumerate(component_notes):
            if clip_note._tied: # For tied notes processing
                ...
                if note_tied_index >= 0:
                    ...
                    note_extended_durations[index] = Fraction(-1)   # Negative extended duration, means to be excluded
        ...
        match operand:
            ...
            case dict():
                if all(isinstance(item, oe.Element) for item in operand.values()):
                    for index, item in operand.items():
                        if isinstance(index, int) and index >= 0 and index < len(self.elements_unmasked()):
                            self.elements_unmasked()[index] = item.copy()
                else:   # Not for me
                    for item in self.elements_unmasked():
                        item << operand
            case Composition():
    ...
    def __isub__(self, operand: any) -> Self:
        match operand:
            case Clip():
                return self._delete(operand.elements_unmasked())
            case oe.Element():
                return self._delete([ operand ])
            case list():
                if all(isinstance(item, oe.Element) for item in operand):
        ...
        match operand:
            ...
            case int():
                if operand > 1:
                    ...
                    remaining_elements: list[oe.Element] = self.elements_unmasked()
            ...
            case ra.Duration() | ra.TimeValue() | float():
                ...
                for first_element in self.elements_unmasked():
                    ...
                    if segment_duration_beats < group_length:
                        ...
                        if isinstance(operand, ra.Duration):    # Duration only splits once
            ...
            case ra.Position() | ra.TimeUnit(): # Single point split if Position
                new_elements: list[oe.Element] = []
                for existent_element in self.elements_unmasked():
...
class Section(Composition):
    ...
    def __setitem__(self, index: Any, value: 'Clip') -> Self:
        """
        Read and Write method
        """
    ...
    def getPlotlist(self, from_part: bool = False) -> list[dict]:
        """
        Returns the plotlist for a given Position.
...
class Part(Composition):
    ...
    def __getitem__(self, index: Any) -> 'Section':
        return super().__getitem__(index)
    
    def __setitem__(self, index: Any, value: 'Section') -> Self:
        """
        Read and Write method
        """
        if isinstance(value, Section):
            target_section: Section = self[index]
            if isinstance(target_section, Section) and value is not target_section:
                self._replace(target_section, value)    # Makes sure it propagates
