...
single_clock = Clock() >> Save("json/_Save_1.1_jsonMidiCreator.json")
...
multi_notes = Rest(NoteValue(Step(3*4 + 2))) >> first_note * 3 << Track("Piano") >> Play(0) >> Save("json/_Save_1.2_sequence.json") >> Export("json/_Export_1.1_sequence.json")
...
all_elements >> Play() >> Export("json/_Export_1.2_all_elements.json")  # IT'S GONNA BE A SONG EXPORT !!
