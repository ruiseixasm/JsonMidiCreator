...
final_clip = Clip()
first_clip = First(1)
left_clip = mutated_clip.copy().cut(Position(0), Position(Steps(12)))
division = Division()

for _ in range(400):
    mutated_clip <<= duration_mutation
    division += left_clip / mutated_clip.copy().stack().link().trim(Length(Steps(4))) >> minimum_notes >> length_condition >> total_plays
