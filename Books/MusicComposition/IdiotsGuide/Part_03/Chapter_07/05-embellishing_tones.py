...
embellishing += Note("B", 1/8, OLD_Position(2)) * 4 - Foreach(0, 1, 0, 1)
embellishing >> Link() >> Rest() >> Play()

embellishing -= embellishing | Equal(Beats(1), Steps(2))
embellishing += Note("A", 1/8, OLD_Position(0, Beats(1))) * 2 + Foreach(0, 2)
embellishing += Note("A", 1/8, OLD_Position(2, Beats(1))) * 2 + Foreach(2, 0)
embellishing >> Link() << Get(NoteValue())**NoteValue() >> Rest() >> Play()

embellishing -= embellishing | Equal(Measures(0), Measures(2))**Greater(Beats(0))
embellishing += Note("E", 1/8, OLD_Position(0, Beats(3))) * 2 + Foreach(2, 0)
embellishing += Note("D", 1/8, OLD_Position(2, Beats(3))) * 2 + Foreach(2, 0)
