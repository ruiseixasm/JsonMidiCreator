...
long_notes: Sequence = Note("D", 1/1) * 4 + Note("D", 1/2, Position(Measure(1), Beat(2))) >> Link()
long_notes << Foreach(6, 5, 1, 2, 6)**Degree() << Nth(3, 4)**Octave(5)
long_notes_pl = Playlist(long_notes)
long_notes_pl >> Rest() >> Play()

short_notes: Sequence = long_notes - (long_notes.copy() | Measure(2)) + (long_notes.copy() << NoteValue(1/4) | Measure(2)) * 4 >> Link()
...
repeated_notes: Sequence = short_notes.copy() + Equal(Measure(2))**Even()**Position(NoteValue(1/16)) >> Link()
