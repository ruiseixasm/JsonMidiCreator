...
rhythmic_motif: Sequence = Note("G", 1/8, Position(NoteValue(1/8))) * 4 >> Link(True)
rhythmic_motif - Equal(Measure(1))**2
rhythmic_motif >> Rest() >> Play()

staff << TimeSignature(4, 4) << KeySignature(-2)   # Same as 'bb'

rhythmic_motif_1: Sequence = Note("B") * 4 << Foreach(1/8, 1/4, 1/8, 1/2)**NoteValue() >> Stack()
rhythmic_motif_2: Sequence = Note("A") * 6 << Foreach(1/8, 1/4, 1/8, 1/8, 1/4, 1/8)**NoteValue() >> Stack()
