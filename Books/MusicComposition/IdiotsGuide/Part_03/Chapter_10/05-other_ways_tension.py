...
staff << "#"
Key() % str() >> Print()
notes_B     = Note("B") * 3 << Foreach((0), (0, Beats(2)), (1, Beats(1)))**OLD_Position()
notes_A     = Note("A") * 2 << Foreach(OLD_Position(0, Beats(1)), OLD_Position(1, Beats(2)))
notes_C5    = Note("C", 5) * 2 << Foreach(OLD_Position(0, Beats(3)), OLD_Position(2))
notes_D5    = Note("D", 5) * 3 << Foreach(OLD_Position(1), OLD_Position(1, Beats(3)), OLD_Position(2, Beats(2)))
notes_E     = Note("E") * 1 << OLD_Position(2, Beats(1))
notes_F5    = Note("F", 5) * 1 << OLD_Position(2, Beats(3))
notes_G     = Note("G") * 1 << OLD_Position(3)
notes_G5    = Note("G", 5) * 1 << OLD_Position(3, Beats(1))
notes_E5    = Note("E", 5) * 1 << OLD_Position(3, Beats(2))
