...
violin_1 = Note(Channel(11), Octave(5), 1/1) / 4 << Name("Violin 1") << Equal(Measure(1))**(Minor(), Degree(-1))
violin_1 << Foreach(1, 3, 1, 7)**Degree()

violin_2 = Note(Channel(12), Velocity(80), 1/1) / 4 << Name("Violin 2") << Equal(Measure(1))**(Minor(), Degree(-1))
violin_2 << Foreach(5, 1, 6, 5)**Degree()

viola = Note(Channel(13), 1/1) / 4 << Name("Viola") << Equal(Measure(1))**(Minor(), Degree(-1))
viola << Foreach(3, 5, 4, 2)**Degree()

cello = Note(Channel(14), Octave(3), 1/1, Velocity(70)) / 4 << Name("Cello") << Equal(Measure(1))**(Minor(), Degree(-1))
