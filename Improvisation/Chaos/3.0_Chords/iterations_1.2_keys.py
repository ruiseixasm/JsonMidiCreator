...
modulus = Modulus(Period(7))   # Works like a cyclic picker
flipper = Flipper(Period(7), Split(5))   # Works like a cyclic picker

def new_clip(clip: Clip) -> Clip:
    clip << Input(flipper)**Choice(4, 3)**Octave()
    return clip

# Equivalent to 3 Measures and 2 Beats
triads = Chord(1/2) * 7 << Input(modulus)**Choice("C", "D", "E", "F", "G", "A", "B")
